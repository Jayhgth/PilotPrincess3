-- A four-year integrated high-school and college plan can legitimately exceed
-- the original 40-row guard. Keep a bounded limit while allowing the complete
-- deterministic batch to be written atomically and restored by Pilot undo.
create or replace function public.replace_pilot_course_schedule(
  p_course_rows jsonb,
...
grant execute on function public.replace_pilot_course_schedule(jsonb, smallint[]) to authenticated;
