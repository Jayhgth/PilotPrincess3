-- Pilot and the manual UI share an atomic boundary for coherent edits to
-- existing student-owned plan rows. Catalog and prerequisite policy remains in
-- the application; this function enforces ownership, transcript locks, and
...
begin
  ...
  if jsonb_typeof(p_rows) <> 'array' then
    raise exception 'Course edits must be an array.';
  end if;
  ...
  if exists (
    ...
    join public.plan_courses course
      on course.id = (item ->> 'id')::uuid
     and course.user_id = target_user_id
    where course.source_review_item_id is not null
       or course.status = 'completed'
  ) then
    raise exception 'Completed and transcript-backed courses cannot be edited here.';
  end if;
  ...
  return jsonb_build_object('updated_count', updated_count, 'rows', result);
end;
$$;
