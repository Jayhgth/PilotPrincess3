...
- One lazy global assistant rail replaces repeated metadata-heavy review panels. A top-right control keeps it available throughout the workspace. On wide screens it has a persisted t3code-style resize rail and can detach into a draggable, natively resizable floating panel; narrower screens use a focused full-width overlay without desktop movement controls. The timeline uses compact user bubbles, unboxed sanitized GFM answers, streaming progress with live elapsed time, automatically folded completed reasoning labeled with its measured duration, readable tool calls, and focused approval cards.
- Conversations, messages, sanitized events, and tool calls persist in RLS-protected Supabase tables. A reloaded or newly opened page can continue the same conversation. Active-history rows can be archived without deletion and later restored from the centered Pilot settings dialog.
...
- `pnpm test`: 132 tests passing across 18 files, including assistant Markdown sanitization and turn timing, assistant image boundaries, generated next-step reconciliation, inactive-experience workload exclusion, bounded activity reduction, catalog eligibility, math progression, cached SMCCD prerequisite evaluation, associate-degree evidence, transcript GPA, the conservative UC GPA lens, Codex model/tool/knowledge/review boundaries, unresolved discipline rules, and the selected Path contract.
- `pnpm build`: passing with the Astro Node server output.
- Authenticated browser QA verified the compact 340-438 px docked states, drag resizing with matching workspace margin, width persistence after reload, free movement in floating mode, the centered Connection/Archive/Interface settings dialog, archive controls, and the full-width 390 px mobile fallback. Light and dark themes were inspected and the original light preference was restored. No conversation was archived during QA, so Jay's history was not changed.
...
- Linked migration history matches through `20260711023000_admin_testing_tools.sql`. The private attachment infrastructure and administrator role/reset boundary are applied. Linked schema lint reports no errors and the remote database is up to date.
...
- The milestone gate (`lint`, typecheck, 132 unit tests, production build) passes. The authenticated assistant rail was rechecked after a clean dev-server restart; no test message was sent to Codex or persisted to Jay's history. The unrelated full Playwright, palette, SMCCD catalog, and linked-schema gates were not required for this UI-only change.
