...
- The public sign-in hero now uses a separately loaded, locally owned React Bits Floating Lines adaptation tuned to the product's graphite and burgundy palette. One full-page visual layer with a continuous graphite base continues behind both the story and authentication task; a single uniform graphite veil restrains the line glow without exposing the grid boundary. The sign-in task and CSS fallback render independently of the Three.js chunk, while the shader pauses when hidden or off-screen, limits pixel density, uses gentle fine-pointer response, and renders a static reduced-motion frame.
