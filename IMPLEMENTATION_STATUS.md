...
The end-to-end local MVP is implemented in Astro and backed by Supabase project `zqkzgmwptdsaqbzrjngt`. It supports open email registration, onboarding, deterministic transcript import, one course workspace, d.tech graduation tracking, SMCCD course and AA/AS discovery, student decision tools, and a persistent server-only Pilot Assistant.
...
- UC/CSU A-G is no longer a product scope. Its Graduation tab, progress calculator, tests, routing, and UC-specific GPA estimate were removed; Graduation now contains only d.tech diploma and selected associate-degree evidence. The transcript parser still preserves the printed UC course-list marker solely so it is not mistaken for Honors.
- Deterministic science lanes, Level 3 language sequence completion, and exact GPA reproduction for the supplied PDF.
- Selected four-year Path Overview aligned to the Done/In progress/Planned course model.
- GPA is now a schedule calculator rather than a static lens page. It locks completed transcript evidence, applies student-supplied grades only to current/planned rows, compares weighted and unweighted results, exposes the saved-schedule all-A ceiling, and solves the lowest uniform grade band for a target.
...
- A focused authenticated Graduation check confirmed exactly two scopes (`d.tech diploma`, `Associate degree`), no A-G text or route, no UC estimate on GPA planner, and zero desktop horizontal overflow.
...
- Linked migration history matches through `20260711028000_remove_ag_architecture.sql`. Source-backed enrollment policies, per-student guardrails, provider provenance, and the two-scope Graduation guidance are applied; linked schema lint reports no errors and a dry-run confirms the remote database is up to date.
...
- The milestone gate (`lint`, typecheck, 148 unit tests, production build) passes without warnings. This pass exercised the authenticated transcript audit and the simplified Graduation/GPA architecture without changing academic evidence. The unrelated full Playwright, palette, and SMCCD catalog gates were not required for this milestone.
