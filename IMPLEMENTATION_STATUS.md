...
- Authentication now sits in a compact React Bits Spotlight Card on a quieter page canvas instead of filling the entire right column. The burgundy spotlight follows fine-pointer movement without React re-renders, relocates to keyboard focus, and pairs with one reduced-motion-safe entrance transition.
