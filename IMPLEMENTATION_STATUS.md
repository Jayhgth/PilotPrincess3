...
- Authenticated browser QA verified the docked rail beside the populated Overview, the repaired compact context/review row, the attachment control, a pasted synthetic PNG preview, the image-aware prompt, and removal without sending. No personal image or live image turn was submitted during QA. The previously verified live `gpt-5.6-luna` read, persisted conversation, readable activity, and rejected write still cover the underlying conversation boundary.
