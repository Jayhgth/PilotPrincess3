...
- The composer remains interactive during a turn. Up to five text/image follow-ups can queue and run automatically in order; each can be removed or promoted to Steer next. Stop is a separate control, and cancelled turns persist a readable lifecycle record rather than disappearing from history.
...
- Authenticated browser QA verified the docked/overlay boundary, width persistence, floating mode, centered settings, archive controls, and full-width mobile fallback. The rail overlays at 420px through 1439px; at 1440px it docks at 360px while preserving a non-overflowing 844px workspace area. Light and dark themes were inspected and the original light preference was restored. No conversation was archived during QA, so Jay's history was not changed.
...
- The milestone gate (`lint`, typecheck, 139 unit tests, production build) passes. Authenticated browser QA measured zero assistant overflow at 1300px overlay, 1440px docked, and 390px mobile widths; the workspace also remained non-overflowing at both desktop boundaries. The temporary viewport and panel state were restored, and no message was sent to Codex or persisted to Jay's history. The unrelated full Playwright, palette, SMCCD catalog, and linked-schema gates were not required for this assistant milestone.
