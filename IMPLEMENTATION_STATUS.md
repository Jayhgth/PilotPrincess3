...
- One lazy global assistant rail replaces repeated metadata-heavy review panels. A top-right control keeps it available throughout the workspace. On wide screens it has a persisted t3code-style resize rail and can detach into a draggable, natively resizable floating panel; narrower screens use a focused full-width overlay without desktop movement controls. The timeline uses compact user bubbles, unboxed sanitized GFM answers, copyable timestamped messages, retry-as-new-turn, streaming progress with live elapsed time, automatically folded completed reasoning labeled with its measured duration, grouped older tool calls, readable active tools, and focused approval cards.
...
- The milestone gate (`lint`, typecheck, 137 unit tests, production build) passes. Authenticated browser QA rechecked the assistant rail in light/dark desktop states and the 390 px full-width fallback after a clean dev-server restart. Draft persistence was tested and the temporary draft and theme change were restored; no message was sent to Codex or persisted to Jay's history. The unrelated full Playwright, palette, SMCCD catalog, and linked-schema gates were not required for this assistant milestone.
