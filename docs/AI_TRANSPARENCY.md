...
Onboarding presents Codex as optional. Connecting requires a student-owned consent checkbox, a successful live test, and an allowlisted model selection. GPT-5.6 Luna with Light reasoning is recommended; the student may choose GPT-5.5 or GPT-5.4 Mini or continue without AI. The universal Settings page owns the current opt-in, allowlisted model, Light/Standard/Deep reasoning level, connection test, consent, and archived conversations. The same compact allowlisted model picker is available in the chat composer. Changing a model or reasoning level does not expand tool access or bypass the mandatory review boundary.
...
- add an exact schedule batch after showing the returned courses; the same validated batch is sent directly to an independent safety reviewer;
...
- update ordinary student and planning settings plus the connected Pilot model and reasoning level, excluding AI consent, authentication, account lifecycle, the mandatory safety-review policy, and administrator state;
...
Every write is an exact proposal first and follows one mandatory path. A separate isolated Codex reviewer sees the student's request, action name, exact arguments, and explanation, then returns `approve` or `deny` with a bounded risk label and student-readable summary. An approval executes automatically; a denial is recorded as not applied. There is no student-selectable manual/automatic review mode and no approval card that can bypass the reviewer.

Risk labels describe impact but do not create a second approval step. Explicit removals, grade edits, and moves to Done can be approved when the request and exact arguments match. Ambiguous, broader-than-requested, unsupported, or unverifiable proposals are denied automatically. A reviewer failure also declines the proposal instead of leaving it pending. Approved changes still execute the same server-side RLS, eligibility, prerequisite, transcript-lock, and validation rules as the normal product UI; neither the assistant nor reviewer can bypass them.
...
[t3code](https://github.com/pingdotgg/t3code) is the interaction reference: a chat timeline, folded agent work, readable tools, and separate reviewer routing. Pilot Princess uses a separately prompted reviewer and risk framework rather than blind full access, with one mandatory autonomous result: apply or decline. It uses the official TypeScript `@openai/codex-sdk`, not t3code's app-server transport.
...
Selected conversation history, tool results, and images explicitly attached to the current message are sent to OpenAI Codex. The student's triggering message and each exact proposed action are also sent through a separate Codex turn for safety review. Provider-side handling follows the configured OpenAI account. Temporary local runtimes are deleted after each turn, while the product conversation, private image attachments, reviewer summary, and sanitized activity persist in Supabase until an application retention or deletion policy removes them. Per-user RLS and private storage policies prevent another student from reading or changing those records.
...
- The mandatory independent safety reviewer applies an approved exact proposal or declines it without asking for confirmation.
