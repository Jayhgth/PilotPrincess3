...
SMCCCD Board Policy 6.26 provides reciprocity among Cañada, CSM, and Skyline: a course satisfying a GE, elective, statutory, or specific-area requirement at one district college is accepted for that same requirement by the others, and a completed GE pattern transfers wholesale. Major-field credit transfers, but the destination college's exact major requirements still control the award. The engine preserves the course's source-campus designation, applies this reciprocity, uses the selected college's area/unit structure, and keeps residency, catalog rights, waivers, external transcripts, substitutions, and final award certification outside automatic completion.
