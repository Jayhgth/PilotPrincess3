...
Pilot schedule construction treats every bookmarked degree and the selected high-school diploma as one constrained planning problem. Candidate college courses are ranked by shared major/GE coverage, verified high-school equivalency, prerequisite order, degree-unit progress, major and interest alignment, GPA weighting, balanced term load, and the saved provider limit. The selected school's planning profile controls eligible college grades, required on-campus areas, minimum high-school load, and normal grade flow. Automatic generation cannot stack duplicate core-area fillers in one grade to repair missed prior-year credit. For d.tech, Personal Development is earned through intersession courses; Introduction to Prototyping and Fabrication is not Personal Development credit.
