...
2. **Courses** owns Done, In progress, and Planned course organization plus exact selected-school and college course selection. It does not own degree selection or degree progress.
...
Statewide search includes every eligible public and charter high school, but support is explicit: **discovery** means identity only, **partial** means at least one reviewed catalog, diploma, or planning source, and **complete** means all three are verified. Onboarding and the Overview surface show this state. Unsupported evidence stays unavailable rather than being inferred from another school.

Associate-degree discovery uses one search field rather than stacked mode, award, and college filters. Degrees appear as equal-size cards in a horizontal rail; students mark any number with a red bookmark, with no artificial primary-degree rank. Major requirements stay attached to the selected degree and use full-row completed, planned, partial, and missing tinting. CSM, Skyline, and Cañada use separate 2025-2026 local-degree pattern definitions. The GE page switches the entire pattern, not only its course list, and displays Skyline and CSM graduation competencies separately when they are outside GE. Course eligibility comes from each source college's complete official worksheet and follows district reciprocity without flattening the awarding college's unit, laboratory, or statutory structure. The audit assigns a course to only one local GE area unless an official separate requirement permits reuse.
...
The production interface uses DM Sans and the semantic tokens in `src/styles/global.css` and `src/styles/t3code.css`. Its retained structure follows the useful parts of t3code: a compact fixed navigation rail, dense workbench rows, restrained surfaces, and an inspectable assistant rail. A muted blue-grey is the product accent for navigation, focus, selection, and general actions. Dark mode uses deep neutral graphite. Institution colors appear only in scoped identity; they do not replace the shared product accent.
