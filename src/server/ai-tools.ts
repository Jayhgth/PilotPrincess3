...
import { planVersionDifferences, planVersionMetrics, planVersionRole, planVersionStrategy } from "@/lib/plan-versions";
import { activateOwnedPlanVersion, archiveOwnedPlanVersion, createOwnedPlanVersion, listOwnedPlanVersions, renameOwnedPlanVersion } from "@/lib/plan-version-store";
...
const toolArgumentSchemas = {
  ...
  get_plan_versions: z.object({ include_backups: z.boolean().default(false) }),
  compare_plan_versions: z.object({
    version_ids: z.array(z.uuid()).min(2).max(4)
      .refine((ids) => new Set(ids).size === ids.length, "Plan versions must be unique.")
  }),
  search_california_high_schools: z.object({ query: z.string().trim().min(2).max(100) }),
  search_course_catalog: z.object({
    query: z.string().trim().min(1).max(80),
    source: z.enum(["high_school", "dtech", "college", "smccd", "all"]).default("all"),
    grade_level: gradeSchema.optional()
  }),
  ...
  get_course_schedule_options: z.object({
    respect_recommended_limit: z.boolean().default(true),
    enforce_school_course_counts: z.boolean().default(false),
  ...
  search_college_programs: z.object({
    query: z.string().trim().min(1).max(100),
    college: z.enum(["CSM", "SKY", "CAN", "all"]).default("all"),
    award_type: z.enum(["AA", "AS", "all"]).default("all")
  }),
  ...
  update_plan_courses: z.object({
    patches: z.array(z.object({
      ...
      .refine((value) => value.remove || Object.keys(value).some((key) => !["plan_course_id", "remove"].includes(key)), "Provide at least one course field to update.")).max(80)
      .refine((patches) => new Set(patches.map((patch) => patch.plan_course_id)).size === patches.length, "Plan course IDs must be unique.").default([]),
    additions: z.array(z.object({
      ...
      .refine((entries) => new Set(entries.map((entry) => `${entry.source}:${entry.course_id}`)).size === entries.length, "Added courses must be unique.").default([])
  }).refine((value) => value.patches.length + value.additions.length > 0, "Provide at least one course edit or addition."),
  ...
  create_plan_version: z.object({
    label: z.string().trim().min(1).max(100),
    source_version_id: z.uuid().optional(),
    start_empty: z.boolean().default(false),
    activate: z.boolean().default(true),
    strategy: z.enum(["balanced", "highest_gpa", "degree_overlap", "minimum_courses"]).default("balanced"),
    populate_strategy: z.boolean().default(false)
  }),
  merge_plan_versions: z.object({
    source_version_id: z.uuid(),
    target_version_id: z.uuid(),
    source_course_ids: z.array(z.uuid()).min(1).max(160)
      .refine((ids) => new Set(ids).size === ids.length, "Selected courses must be unique.")
  }).refine((value) => value.source_version_id !== value.target_version_id, "Choose two different plans."),
  activate_plan_version: z.object({ version_id: z.uuid() }),
  rename_plan_version: z.object({ version_id: z.uuid(), label: z.string().trim().min(1).max(100) }),
  archive_plan_version: z.object({ version_id: z.uuid() }),
  ...
  clear_academic_plan: z.object({
    // Never turn an omitted model argument into a destructive default.
...
const ASSISTANT_TOOL_CATALOG: ReadonlyArray<{
  ...
  { name: "get_plan_versions", mutatesData: false, description: "List the student's named four-year plans and optionally recent automatic backups, including stable IDs and course counts. Use before switching, renaming, deleting, restoring, or comparing plans.", arguments: '{"include_backups":boolean}' },
  { name: "compare_plan_versions", mutatesData: false, description: "Compare two to four complete named plans using diploma coverage, each bookmarked degree, projected weighted GPA, course count, peak college units, and exact added or moved courses. The first ID is the baseline; later plans return source course IDs that can be passed directly to merge_plan_versions.", arguments: '{"version_ids":["uuid","uuid"]}' },
  ...
  { name: "search_course_catalog", mutatesData: false, description: "Search the selected high school's approved catalog and/or the selected district's supported college catalog. Returns stable course IDs and never mixes high-school catalogs or college providers.", arguments: '{"query":"string","source":"high_school|college|all","grade_level":9|10|11|12}' },
  ...
  { name: "get_degree_progress", mutatesData: false, description: "Read deterministic requirement-level evidence for one bookmarked supported associate degree. Omit program_id only when one bookmark is sufficient context.", arguments: '{"program_id":"string|optional"}' },
  { name: "get_college_goal", mutatesData: false, description: "Read all bookmarked supported associate degrees.", arguments: "{}" },
  { name: "search_college_programs", mutatesData: false, description: "Search official supported college AA and AS programs by name or program code. Results retain their provider and awarding-college identity and return exact IDs needed to bookmark a degree.", arguments: '{"query":"string","college":"CSM|SKY|CAN|all","award_type":"AA|AS|all"}' },
  { name: "search_smccd_programs", mutatesData: false, description: "Compatibility alias for searching the currently supported SMCCD program provider.", arguments: '{"query":"string","college":"CSM|SKY|CAN|all","award_type":"AA|AS|all"}' },
  ...
  { name: "add_course_schedule", mutatesData: true, description: "Apply the exact integrated high-school and bookmarked-degree schedule returned by get_course_schedule_options to that result's exact named plan. Pass target_plan_version_id and every schedule constraint unchanged. course_ids contains the returned selected-school course IDs; the server deterministically revalidates and atomically includes the returned college-degree portion, so it cannot be silently omitted. Transcript evidence remains locked and replacement is limited to the explicit scope.", arguments: '{"target_plan_version_id":"uuid","course_ids":["uuid"],"respect_recommended_limit":boolean,"enforce_school_course_counts":boolean,"interests":["string"],"rigor":"balanced|advanced|lighter","max_courses_per_term":number|null,"start_grade":9|10|11|12,"starting_math_course":"string|null","starting_language_course":"string|null","include_college_courses":boolean,"replace_existing":boolean,"replace_grade_levels":[9|10|11|12],"objectives":["complete_diploma|maximize_weighted_gpa|maximize_degree_overlap|align_major"]}' },
  ...
  { name: "add_high_school_course", mutatesData: true, description: "Propose adding one approved course from the student's selected high-school catalog to In progress or Planned. An unmet prerequisite is allowed, returned as an advisory, and visibly flagged on the saved plan course.", arguments: '{"course_id":"uuid","status":"current|planned","grade_level":9|10|11|12,"term":"fall|spring|summer|full_year","prerequisite_override_reason?":"string"}' },
  { name: "add_smccd_course", mutatesData: true, description: "Propose adding one college catalog course to In progress or Planned. An unmet prerequisite is allowed, returned as an advisory, and visibly flagged on the saved plan course.", arguments: '{"course_id":"string","status":"current|planned","grade_level":9|10|11|12,"term":"fall|spring|summer|full_year","prerequisite_override_reason?":"string"}' },
  ...
  { name: "update_plan_courses", mutatesData: true, description: "Atomically apply one coherent batch of exact edits, removals, and additions while preserving every unaffected non-transcript row. Unmet prerequisites do not reject the batch; they return advisories and remain visibly flagged on the saved courses.", arguments: '{"patches":[{"plan_course_id":"uuid","remove":boolean,"course_id":"uuid","smccd_course_id":"catalog id","grade_level":9|10|11|12,"term":"fall|spring|summer|full_year","letter_grade":"string|null","credits":number,"college_units":number|null,"is_weighted":boolean,"notes":"string|null","prerequisite_override_reason?":"string"}],"additions":[{"source":"selected_school|smccd","course_id":"catalog id","status":"current|planned","grade_level":9|10|11|12,"term":"fall|spring|summer|full_year","prerequisite_override_reason?":"string"}]}' },
  ...
  { name: "update_student_settings", mutatesData: true, description: "Propose changing ordinary student, interface theme, and connected Pilot model/reasoning settings. Include only fields explicitly requested. The four-year plan always spans grades 9 through 12. This cannot change Pilot opt-in consent, authentication, account lifecycle, or product security boundaries.", arguments: '{"preferred_name?":"string","age?":number|null,"grade_level?":9|10|11|12|null,"graduation_year?":number|null,"tracker_mode?":"full|selected","tracked_requirement_areas?":["english|..."],"ai_model?":"gpt-5.6-luna|gpt-5.5|gpt-5.4-mini","ai_reasoning_effort?":"low|medium|high","ui_theme?":"light|dark"}' },
  ...
  { name: "create_plan_version", mutatesData: true, description: "Create a complete named four-year plan strategy by copying an owned plan version or starting with transcript history only. Activating it preserves the previous plan as another selectable version. Set populate_strategy=true when the student asks for a new planned strategy such as a GPA-maximized or degree-overlap plan; the application will build that plan after creating it.", arguments: '{"label":"string","source_version_id":"uuid|optional","start_empty":boolean,"activate":boolean,"strategy":"balanced|highest_gpa|degree_overlap|minimum_courses","populate_strategy":boolean}' },
  { name: "merge_plan_versions", mutatesData: true, description: "Merge an exact selected set of courses from one owned named plan into another owned plan in the same school workspace. Matching courses adopt the source placement; missing courses are copied. Compare plans first and use the returned source course IDs. The operation is atomic and reversible.", arguments: '{"source_version_id":"uuid","target_version_id":"uuid","source_course_ids":["uuid"]}' },
  { name: "activate_plan_version", mutatesData: true, description: "Open an existing named plan or backup as the active four-year plan without losing the plan currently open.", arguments: '{"version_id":"uuid"}' },
  { name: "rename_plan_version", mutatesData: true, description: "Rename one owned named plan.", arguments: '{"version_id":"uuid","label":"string"}' },
  { name: "archive_plan_version", mutatesData: true, description: "Delete one inactive named plan while preserving a reversible restore action. The active plan cannot be deleted.", arguments: '{"version_id":"uuid"}' },
  ...
  { name: "clear_college_goal", mutatesData: true, description: "Propose removing one supported college degree bookmark.", arguments: '{"program_id":"string"}' },
...
export function assistantToolCatalogPrompt(userMessage?: string) {
  ...
  const compatibilityAliases = new Set<AssistantToolName>([
    "add_dtech_course",
  ...
  return ASSISTANT_TOOL_CATALOG.filter((tool) => !compatibilityAliases.has(tool.name) && (!selectedTools || selectedTools.has(tool.name))).map((tool) => {
    const mutatesData = appCapability(tool.name)?.review !== "none";
    return [
      `- ${tool.name}${mutatesData ? " (exact reversible product change)" : " (read-only)"}`,
      `  ${tool.description}`,
...
export function assistantToolLabel(name: string) {
  return ({
    ...
    get_plan_versions: "Read four-year plans",
    compare_plan_versions: "Compare four-year plans",
    ...
    search_college_programs: "Search college programs",
    ...
    merge_plan_versions: "Merge four-year plans",
...
export function safeParseAssistantToolCall(name: string, argumentsValue: unknown) {
  try {
    return { success: true as const, data: parseAssistantToolCall(name, argumentsValue) };
  } catch (error) {
    const message = error instanceof z.ZodError
      ? error.issues[0]?.message ?? "Invalid tool arguments."
...
function integratedDegreePlan(
  ...
  const currentGrade = Math.max(9, Math.min(12, Number(preferences.startGrade ?? workspace.settings.grade_level ?? 9))) as GradeLevel;
  const endGrade = 12 as GradeLevel;
  ...
  for (const program of programs) {
    ...
    for (const area of local.geAreas) {
      const eligibleCodes = area.eligibleCourseCodes.map(degreeCode);
      // Keep efficient double-counting choices in the GE search even when
      // their catalog code would not land in the first arbitrary three.
      // This is what allows one verified college language, history, science,
      // or mathematics course to advance both the diploma and degree.
      const selected = new Set([
        ...rankCatalogChoices(eligibleCodes, 4),
        ...rankCatalogChoices(eligibleCodes.filter((code) => overlapCodes.has(code)), 8)
      ]);
  ...
  const mathPlacementForRank = (rank: number | null) => {
    if (rank === null || startingMathRank === null) return null;
    const matchingScheduledMath = rows().flatMap((row) => {
      const selectedCourse = row.course_id ? workspace.courses.find((course) => course.id === row.course_id) : null;
      const collegeCourse = row.smccd_course_id ? catalog.find((course) => course.id === row.smccd_course_id) : null;
      const scheduledRank = mathSequenceRankFromText(`${selectedCourse?.course_code ?? collegeCourse?.course_code ?? ""} ${selectedCourse?.name ?? collegeCourse?.title ?? row.custom_course_name ?? ""}`);
      return scheduledRank === rank ? [row] : [];
    }).sort((left, right) => termRank(left.grade_level, left.term === "full_year" ? "fall" : left.term) - termRank(right.grade_level, right.term === "full_year" ? "fall" : right.term));
    const matching = matchingScheduledMath[0];
    if (matching) return {
      grade: matching.grade_level,
      // A college equivalent of a full-year school course belongs after most
      // of that preparation, not years later based on a static start rank.
      term: matching.term === "full_year" ? "spring" as const : matching.term
    };
    if (rank <= 4) return {
      grade: Math.min(endGrade, currentGrade + Math.max(0, rank - startingMathRank)) as GradeLevel,
      term: "fall" as const
    };
    const firstCollegeCalculusGrade = startingMathRank >= 5
      ? currentGrade
      : currentGrade + Math.max(0, 4 - startingMathRank) + 1;
    const semesterOffset = startingMathRank >= 5 ? rank - startingMathRank : rank - 5;
    return {
      grade: (firstCollegeCalculusGrade + Math.floor(Math.max(0, semesterOffset) / 2)) as GradeLevel,
      term: Math.max(0, semesterOffset) % 2 === 0 ? "fall" as const : "spring" as const
    };
  };
  const placementFor = (course: SmccdCourse, options: { prioritizeMajor?: boolean } = {}) => {
    const currentRows = rows();
    const { prioritizeMajor = false } = options;
    ...
    const defaultMathPlacementHint = mathPlacementForRank(courseMathRank);
    const mathPlacementHint = courseCode === "MATH 130" && startingMathRank !== null
      ? {
          grade: Math.min(endGrade, currentGrade + Math.max(1, 4 - startingMathRank)) as GradeLevel,
          term: "fall" as const
        }
      : defaultMathPlacementHint;
    const earliestMathGrade = mathPlacementHint?.grade ?? currentGrade;
    ...
    const requiredMajorCourse = mandatoryMajorCodes.has(courseCode) || prioritizeMajor;
    ...
    return potentialPeriods
      ...
      .sort((left, right) => {
        const hintScore = (period: typeof left) => Number(mathPlacementHint?.grade === period.grade) * 8
          + Number(mathPlacementHint?.term === period.term) * 4
          + Number(placementHint?.grade === period.grade) * 2
          + Number(placementHint?.term === period.term);
        const hinted = hintScore(right) - hintScore(left);
        if (hinted) return hinted;
        if (supportsLaterCourse || requiredMajorCourse) {
      ...
      .find((period) => {
        if (period.grade < earliestMathGrade
          || (mathPlacementHint && termRank(period.grade, period.term) < termRank(mathPlacementHint.grade, mathPlacementHint.term))) {
          return false;
        }
        if (courseMathRank !== null && courseMathRank > 1) {
          const candidateIndex = termRank(period.grade, period.term);
          const highestPriorMathRank = Math.max(0, ...currentRows.flatMap((row) => {
            const rowTerm = row.term === "full_year" ? "fall" : row.term;
            if (termRank(row.grade_level, rowTerm) >= candidateIndex) return [];
            const selectedCourse = row.course_id ? workspace.courses.find((candidate) => candidate.id === row.course_id) : null;
            const collegeCourse = row.smccd_course_id ? catalog.find((candidate) => candidate.id === row.smccd_course_id) : null;
            const rank = mathSequenceRankFromText(`${selectedCourse?.course_code ?? collegeCourse?.course_code ?? ""} ${selectedCourse?.name ?? collegeCourse?.title ?? row.custom_course_name ?? ""}`);
            return rank === null ? [] : [rank];
          }));
          if (highestPriorMathRank < courseMathRank - 1) {
            return false;
          }
        }
        const gradeRule = workspace.planningProfile?.grade_rules[String(period.grade) as `${GradeLevel}`];
        const equivalency = workspace.equivalencies.find((candidate) => candidate.normalized_course_code === degreeCode(course.course_code));
        if (equivalency?.requirement_area === "social_science" && placementHint?.grade && period.grade !== placementHint.grade) {
          return false;
        }
        const targetTotalCourses = preferences.maxCoursesPerTerm ?? gradeRule?.target_total_courses ?? 6;
        const requiredOnCampusAreas = new Set(workspace.planningProfile?.always_high_school_areas ?? []);
        const earlyYearMinimum = period.grade < Math.max(currentGrade, endGrade - 1)
          ? Number(gradeRule?.minimum_high_school_courses ?? 0)
          : 0;
        const reservedHighSchoolCourses = period.term === "summer"
          ? 0
          : Math.max(period.highSchoolCount, requiredOnCampusAreas.size, earlyYearMinimum);
        ...
        if (preferences.enforceSchoolCourseCounts && period.count >= targetTotalCourses) {
          return false;
        }
        if (period.grade < Math.max(currentGrade, endGrade - 1)
          && period.count >= targetTotalCourses
          && (preferences.enforceSchoolCourseCounts || !prioritizeMajor)) {
          return false;
        }
        // School course-count targets shape an ordinary recommendation, but
        // they are not an absolute enrollment rule. An unresolved bookmarked
        // requirement may use one additional slot when prerequisites and the
        // district unit policy still permit it. This prevents a low-value GE
        // choice from permanently crowding out the course needed to finish a
        // degree.
        const laterYear = period.grade >= Math.max(currentGrade, endGrade - 1);
        const effectiveCollegeCapacity = collegeCourseCapacity
          + (!preferences.enforceSchoolCourseCounts
            ? (prioritizeMajor ? 2 : supportsLaterCourse && laterYear ? 1 : 0)
            : 0);
        if (period.collegeCount >= effectiveCollegeCapacity) {
          return false;
        }
        if (preferences.maxCoursesPerTerm && period.count >= preferences.maxCoursesPerTerm) {
          return false;
        }
  ...
  for (let pass = 0; pass < 44 && !currentAudit.complete; pass += 1) {
    ...
    const placedCandidates = candidates.flatMap((course) => {
      ...
      const advancesUnfinishedGoal = currentAudit.neededMajorCodes.has(code)
        || (needsLocalGe && localGeOptionCodes.has(code))
        || (needsSeparateGraduation && separateGraduationOptionCodes.has(code));
      const period = placementFor(course, { prioritizeMajor: advancesUnfinishedGoal });
      if (!period) return [];
      ...
      const curriculumPlacement = equivalencyPlacementHint(course);
      const hasExactCurriculumPlacement = Boolean(curriculumPlacement?.grade && curriculumPlacement?.term);
      const shortlistScore = Number(needsMajor && neededMajorCourse && mandatoryMajorCodes.has(code)) * 2_000_000
        ...
        + Number(hasExactCurriculumPlacement) * 600_000
      ...
      return [{ course, period, shortlistScore, matchesMajorSubjectRule, hasExactCurriculumPlacement }];
    ...
    for (const { course, period, matchesMajorSubjectRule } of placedCandidates) {
      ...
      // Preference and GPA bonuses may break ties between useful courses, but
      // they must never spend a schedule slot on a course that advances no
      // remaining degree, GE, prerequisite, unit, or diploma objective.
      if (marginal <= 0 && structuralBonus <= 0) continue;
  ...
  // A prerequisite-heavy major can unlock only after earlier courses have
  // already occupied the schedule. Reclaim a redundant generated slot instead
  // of abandoning the bookmarked goal or exceeding the district unit limit.
  // Every accepted swap must improve the degree audit while preserving all
  // diploma credit, completed GE/degree outcomes, and prerequisite validity.
  const diplomaSignatureFor = (planRows: PlanCourse[]) => new Map(calculateRequirementProgress(
  ...
  const blockedPrerequisitesFor = (planRows: PlanCourse[]) => new Set(planRows.flatMap((row) => {
    if (!row.smccd_course_id) return [];
    const course = catalog.find((candidate) => candidate.id === row.smccd_course_id);
    if (!course) return [];
    ...
    return evaluation.result.status === "blocked" ? [course.id] : [];
  }));
  const hasCoherentMathSequence = (planRows: PlanCourse[]) => {
    const timeline = planRows.flatMap((row) => {
      const selectedCourse = row.course_id ? workspace.courses.find((course) => course.id === row.course_id) : null;
      const collegeCourse = row.smccd_course_id ? catalog.find((course) => course.id === row.smccd_course_id) : null;
      const rank = mathSequenceRankFromText(`${selectedCourse?.course_code ?? collegeCourse?.course_code ?? ""} ${selectedCourse?.name ?? collegeCourse?.title ?? row.custom_course_name ?? ""}`);
      if (rank === null) return [];
      const term = row.term === "full_year" ? "fall" : row.term;
      return [{ rank, index: termRank(row.grade_level, term) }];
    }).sort((left, right) => left.index - right.index || left.rank - right.rank);
    return timeline.every((item, index) => index === 0
      || (item.rank >= timeline[index - 1]!.rank && item.rank - timeline[index - 1]!.rank <= 1));
  };
  const preservesCompletedGoalOutcomes = (before: ReturnType<typeof audit>, after: ReturnType<typeof audit>) => before.goals.every((goal) => {
    const next = after.goals.find((candidate) => candidate.program_id === goal.program_id);
    if (!next) return false;
    return !(goal.major_complete && !next.major_complete)
      && !(goal.local_ge_complete && !next.local_ge_complete)
      && !(goal.separate_requirements_complete && !next.separate_requirements_complete)
      && !(goal.projected_degree_units >= goal.required_degree_units && next.projected_degree_units < next.required_degree_units);
  });
  const replacementPassLimit = Math.min(48, Math.max(16, candidates.length));
  for (let pass = 0; pass < replacementPassLimit && !currentAudit.complete; pass += 1) {
    const baselineRows = rows();
    const baselineDiploma = diplomaSignatureFor(baselineRows);
    const baselineBlocked = blockedPrerequisitesFor(baselineRows);
    let bestSwap: { victimIndex: number; victimCode: string; course: SmccdCourse; draft: GeneratedDegreeCourse; gain: number } | null = null;
    const neededCandidates = candidates.filter((course) => {
      const code = degreeCode(course.course_code);
      return currentAudit.neededMajorCodes.has(code) && !usedCodes.has(code);
    });
    for (const course of neededCandidates) {
      const candidateCode = degreeCode(course.course_code);
      for (let victimIndex = 0; victimIndex < additions.length; victimIndex += 1) {
        const victim = additions[victimIndex]!;
        const victimCourse = catalog.find((candidate) => candidate.id === victim.smccd_course_id);
        if (!victimCourse) continue;
        const victimCode = degreeCode(victimCourse.course_code);
        if (mandatoryMajorCodes.has(victimCode)
          || prerequisiteSupportCodes.has(victimCode)
          || requestedLanguageCodes.has(victimCode)) continue;
        additions.splice(victimIndex, 1);
        usedCodes.delete(victimCode);
        // First reuse the exact slot that the victim occupied. Asking the
        // ordinary placement search to find another opening after removing a
        // course could still select a different, already-full term and leave
        // a required major elective stranded even though a valid one-for-one
        // replacement existed.
        const searchedPeriod = placementFor(course, { prioritizeMajor: true });
        const replacementPeriods = [
          { grade: victim.grade_level, term: victim.term },
          ...(searchedPeriod ? [{ grade: searchedPeriod.grade, term: searchedPeriod.term }] : [])
        ].filter((period, index, all) => period.term !== "full_year"
          && all.findIndex((candidate) => candidate.grade === period.grade && candidate.term === period.term) === index) as Array<{ grade: GradeLevel; term: "fall" | "spring" | "summer" }>;
        for (const period of replacementPeriods) {
          const equivalency = workspace.equivalencies.find((candidate) => candidate.normalized_course_code === candidateCode);
          const collegeUnits = Number(course.units_max ?? course.units_min);
          const draft: GeneratedDegreeCourse = {
            smccd_course_id: course.id,
            grade_level: period.grade,
            ...
            term: period.term,
            status: period.grade === currentGrade ? "current" : "planned",
            credits: resolveCollegeHighSchoolCredits({ collegeUnits, storedHighSchoolCredits: null, equivalencyHighSchoolCredits: equivalency?.high_school_credits, normalizedCourseCode: candidateCode }).credits,
            college_units: collegeUnits,
            college_provider_code: "SMCCD",
            is_weighted: true,
            mapping_verified: Boolean(equivalency),
            requirement_area_override: equivalency?.requirement_area ?? null,
            notes: "Integrated bookmarked-degree course replacing a redundant generated option while preserving verified outcomes."
          };
          ...
          if (gain > 0
            && preservesDiploma
            && !introducesBlockedPrerequisite
            && enrollmentAccepted
            && hasCoherentMathSequence(nextRows)
            && preservesCompletedGoalOutcomes(currentAudit, nextAudit)
            && (!bestSwap || gain > bestSwap.gain)) {
            bestSwap = { victimIndex, victimCode, course, draft, gain };
          }
          additions.pop();
        }
        additions.splice(victimIndex, 0, victim);
        usedCodes.add(victimCode);
      }
    }
    if (!bestSwap) break;
    additions.splice(bestSwap.victimIndex, 1);
    usedCodes.delete(bestSwap.victimCode);
    additions.push(bestSwap.draft);
    usedCodes.add(degreeCode(bestSwap.course.course_code));
    currentAudit = audit(rows());
  }
  ...
  return {
    additions,
    complete: currentAudit.complete,
    goals: currentAudit.goals.map((goal) => ({
      ...goal,
      unresolved_major_course_options: [...new Set(goal.unresolved_major_details.flatMap((detail) => detail.remaining_course_options))]
    }))
  };
}
...
export function normalizedCatalogSearchText(value: string | null | undefined) {
  return normalizedScheduleText(value)
    .replace(/\bpre[ -]?calc(?:ulus)?\b/g, "precalculus")
    .replace(/\bcalc(?:ulus)?\s*([123])\b/g, "calculus $1")
    .replace(/\bcalc\b/g, "calculus")
    .replace(/\beng(?:l)?\s*(c?\d{3,4})\b/g, "engl $1")
...
function batchSequenceIdentity(course: SmccdCourse) {
  const title = normalizedCatalogSearchText(course.title);
...
function scheduleWorkspaceWithPlacementAdjustments(
  ...
  const targetGrade = (preferences.startGrade ?? workspace.settings.grade_level ?? 9) as GradeLevel;
...
function pruneRedundantGeneratedSchedule(
  ...
  const bookmarkedRequirementIds = new Set(workspace.degreeRequirements
    ...
    .map((requirement) => requirement.id));
  const bookmarkedRequiresMath = workspace.degreeRequirementCourses.some((option) => bookmarkedRequirementIds.has(option.requirement_id)
    && /^MATH\s/i.test(option.course_code));
  ...
  const prerequisiteFailures = (rows: PlanCourse[]) => new Set(rows.flatMap((row) => {
    ...
    if (!course) return [];
    const evaluation = evaluateSmccdPlannerPrerequisites(course, { gradeLevel: row.grade_level, term: row.term, instanceId: row.id }, workspace.degreeCatalogCourses, rows, workspace.courses);
    const notReady = evaluation.result.status === "blocked"
      || (evaluation.result.status === "needs_review" && !explicitSmccdPrerequisitesReady(
        course,
        { gradeLevel: row.grade_level, term: row.term },
        rows,
        workspace.degreeCatalogCourses,
        workspace.courses
      ));
    return notReady && !/student-provided|placement override/i.test(row.notes ?? "") ? [row.smccd_course_id] : [];
  }));
  ...
  const preservesPolicy = (rows: PlanCourse[]) => {
    for (const grade of ([9, 10, 11, 12] as GradeLevel[]).filter((value) => value >= startGrade)) {
      ...
      for (const term of ["fall", "spring"] as const) {
        const highSchoolCount = rows.filter((row) => row.grade_level === grade
          && !row.smccd_course_id
    ...
    if (bookmarkedRequiresMath) {
      const mathRanks = rows.flatMap((row) => {
        ...
        return rank === null ? [] : [{
          rank,
          index: (row.grade_level - 9) * 3 + (row.term === "spring" ? 1 : row.term === "summer" ? 2 : 0)
        }];
      }).sort((left, right) => left.index - right.index || left.rank - right.rank);
      for (let index = 1; index < mathRanks.length; index += 1) {
        if (mathRanks[index]!.rank < mathRanks[index - 1]!.rank
          || mathRanks[index]!.rank - mathRanks[index - 1]!.rank > 1) return false;
      }
    }
    return true;
  };
  ...
  const selectedCollegeCodes = new Set(college.flatMap((row) => {
    const course = collegeById.get(row.smccd_course_id);
    const code = course ? normalizeCollegeCourseCode(course.course_code) : null;
    return code ? [code] : [];
  }));
  const rowOrder = (row: Pick<PlanCourse, "grade_level" | "term">) => (row.grade_level - 9) * 3
    + (row.term === "spring" ? 1 : row.term === "summer" ? 2 : 0);
  const selectedPrerequisiteCodes = new Set(college.flatMap((dependentRow) => {
    const course = collegeById.get(dependentRow.smccd_course_id);
    if (!course) return [];
    return course.prerequisites.flatMap((clause) => explicitSmccdPrerequisiteCodes(clause))
      .filter((code) => selectedCollegeCodes.has(code))
      // A prerequisite scheduled after its dependent course is not support;
      // protecting it from pruning preserved backward math such as
      // Precalculus after Calculus II.
      .filter((code) => college.some((candidateRow) => {
        const candidateCourse = collegeById.get(candidateRow.smccd_course_id);
        return normalizeCollegeCourseCode(candidateCourse?.course_code) === code
          && rowOrder(candidateRow as PlanCourse) < rowOrder(dependentRow as PlanCourse);
      }));
  }));
  for (const candidate of collegeCandidates) {
    const index = college.findIndex((row) => row.smccd_course_id === candidate.smccd_course_id);
    if (index < 0) continue;
    const candidateCourse = collegeById.get(candidate.smccd_course_id);
    const candidateCode = candidateCourse ? normalizeCollegeCourseCode(candidateCourse.course_code) : null;
    if (candidateCode && selectedPrerequisiteCodes.has(candidateCode)) continue;
...
function generateValidatedSchedule(
  ...
  preferences: { interests: string[]; rigor: "balanced" | "advanced" | "lighter"; maxCoursesPerTerm: number | null; enforceSchoolCourseCounts?: boolean; startGrade?: GradeLevel; startingMathCourse?: string | null; startingLanguageCourse?: string | null; includeCollegeCourses?: boolean; replaceExisting?: boolean; replaceGradeLevels?: GradeLevel[]; objectives?: string[] } = { interests: [], rigor: "balanced", maxCoursesPerTerm: null }
) {
  ...
  // Establish the selected-school core before fitting college work around it.
  // Planning college courses first made the school pass repair backward from
  // an assumed Calculus placement, compressing several prerequisite levels
  // into one year. The later integrated pass owns all degree/overlap work.
  workspace = {
    ...workspace,
  ...
  const bookmarkedProgramIds = new Set(workspace.collegeGoals.map((goal) => goal.program_id));
  const bookmarkedRequirementIds = new Set(workspace.degreeRequirements
  ...
  const currentGrade = Math.max(9, Math.min(12, Number(preferences.startGrade ?? workspace.settings.grade_level ?? 9))) as GradeLevel;
  const finalGrade = 12 as GradeLevel;
  ...
  const tryAddRequiredCourse = (grade: GradeLevel, area: string, namePattern?: RegExp, requiredMathRank?: number) => {
    ...
    const ranked = workspace.courses
      ...
      .filter((course) => !course.grade_levels.length
        || course.grade_levels.includes(grade)
        || (requiredMathRank !== undefined && course.grade_levels.every((listedGrade) => listedGrade < grade)))
    ...
    for (const course of ranked) {
      ...
      const eligibility = selectedSchoolCatalogEligibility(course, grade, planWithAccepted, workspace.courses, { schoolSlug: workspace.school.slug });
      const laterGradeSequenceRepair = requiredMathRank !== undefined
        && eligibility.reason === "outside_grade"
        && course.grade_levels.length > 0
        && course.grade_levels.every((listedGrade) => listedGrade < grade);
      if (!eligibility.eligible && !laterGradeSequenceRepair) continue;
      const prerequisite = evaluateSelectedSchoolPlannerPrerequisites(course, { gradeLevel: grade, term }, workspace.courses, planWithAccepted, workspace.plannedSmccdCourses, workspace.equivalencies);
      if (prerequisite.result.status === "blocked") continue;
      const addition = {
        course_id: course.id,
  ...
  for (const grade of planningGrades) {
    ...
    if (effectiveRigor === "advanced" || bookmarkedRequiresMath) {
  ...
  const pruned = pruneRedundantGeneratedSchedule({ ...workspace, planCourses: basePlanCourses }, deduplicated, [], {
    startGrade: preferences.startGrade,
    startingMathCourse: preferences.startingMathCourse,
    startingLanguageCourse: preferences.startingLanguageCourse
  });
  const repairedSchoolAdditions = [...pruned.schoolAdditions];
  if (bookmarkedRequiresMath) {
    const courseById = new Map(workspace.courses.map((course) => [course.id, course]));
    for (let repair = 0; repair < 6; repair += 1) {
      const timeline = [
        ...basePlanCourses,
        ...repairedSchoolAdditions.map((row, index) => generatedPlanCourseRow(workspace, row, index)),
        ...pruned.degreeAdditions.map((row, index) => generatedDegreeCourseRow(workspace, row, index))
      ].flatMap((row) => {
        const selectedCourse = row.course_id ? courseById.get(row.course_id) : null;
        const collegeCourse = row.smccd_course_id ? workspace.degreeCatalogCourses.find((course) => course.id === row.smccd_course_id) : null;
        const rank = mathSequenceRankFromText(`${selectedCourse?.course_code ?? collegeCourse?.course_code ?? ""} ${selectedCourse?.name ?? collegeCourse?.title ?? row.custom_course_name ?? ""}`);
        return rank === null ? [] : [{ row, rank }];
      }).sort((left, right) => {
        const index = (row: PlanCourse) => (row.grade_level - 9) * 3 + (row.term === "spring" ? 1 : row.term === "summer" ? 2 : 0);
        return index(left.row) - index(right.row) || left.rank - right.rank;
      });
      const gapIndex = timeline.findIndex((item, index) => index > 0 && item.rank - timeline[index - 1]!.rank > 1);
      if (gapIndex < 1) break;
      const previous = timeline[gapIndex - 1]!;
      const next = timeline[gapIndex]!;
      const missingRank = previous.rank + 1;
      const targetGrade = Math.min(next.row.grade_level, Math.max(previous.row.grade_level + 1, currentGrade)) as GradeLevel;
      const usedCourseIds = new Set([
        ...basePlanCourses.map((row) => row.course_id),
        ...repairedSchoolAdditions.map((row) => row.course_id)
      ].filter((id): id is string => Boolean(id)));
      const repairCourse = workspace.courses
        .filter((course) => !usedCourseIds.has(course.id) && mappedAreasByCourse.get(course.id)?.has("math"))
        .filter((course) => mathSequenceRankFromText(`${course.course_code ?? ""} ${course.name}`) === missingRank)
        .filter((course) => !course.grade_levels.length
          || course.grade_levels.includes(targetGrade)
          || course.grade_levels.every((listedGrade) => listedGrade < targetGrade))
        .sort((left, right) => Number(right.is_weighted) - Number(left.is_weighted) || left.name.localeCompare(right.name))[0];
      if (!repairCourse) break;
      const term: PlanCourse["term"] = repairCourse.term_type === "year" ? "full_year" : "fall";
      const planRows = [
        ...basePlanCourses,
        ...repairedSchoolAdditions.map((row, index) => generatedPlanCourseRow(workspace, row, index)),
        ...pruned.degreeAdditions.map((row, index) => generatedDegreeCourseRow(workspace, row, index))
      ];
      const prerequisite = evaluateSelectedSchoolPlannerPrerequisites(repairCourse, { gradeLevel: targetGrade, term }, workspace.courses, planRows, workspace.plannedSmccdCourses, workspace.equivalencies);
      if (prerequisite.result.status === "blocked") break;
      repairedSchoolAdditions.push({
        course_id: repairCourse.id,
        grade_level: targetGrade,
        school_year: schoolYearForGrade(workspace.settings.graduation_year ?? new Date().getFullYear() + 3, targetGrade),
        term,
        status: targetGrade === currentGrade ? "current" : "planned",
        credits: repairCourse.credits,
        college_units: repairCourse.college_units,
        college_provider_code: null,
        is_weighted: repairCourse.is_weighted,
        mapping_verified: true,
        user_edited: false
      });
    }
  ...
  const degreeRefinement = preferences.includeCollegeCourses === false
    ...
    : integratedDegreePlan(refinementWorkspace, enrollmentPolicy, {
        ...
        startingLanguageCourse: preferences.startingLanguageCourse,
        maxCoursesPerTerm: effectiveMaxCoursesPerTerm,
        enforceSchoolCourseCounts: preferences.enforceSchoolCourseCounts,
        interests: preferences.interests,
        objectives: preferences.objectives
      });
  // Rebuild the college portion from the pruned high-school plan instead of
  // treating the first-pass college choices as immutable workspace rows. A
  // seeded refinement could detect an unfinished bookmarked requirement but
  // could not replace the lower-value course occupying its only valid slot.
  const finalPruned = pruneRedundantGeneratedSchedule(
    { ...workspace, planCourses: basePlanCourses },
    repairedSchoolAdditions,
    degreeRefinement.additions,
    {
      startGrade: preferences.startGrade,
      startingMathCourse: preferences.startingMathCourse,
      startingLanguageCourse: preferences.startingLanguageCourse
  ...
  return {
    ...
    degreeAdditions: finalDegreeAdditions,
    degreeRows: finalDegreeAdditions.map((row, index) => generatedDegreeCourseRow(workspace, row, index)),
    degreeGoals: degreeRefinement.goals,
    degreeComplete: degreeRefinement.complete,
...
function analyzeGeneratedSchedule(
  ...
  const sourceReadiness = {
    ...
    evidence_ready: workspace.courses.length > 0 && placementReadyCourseIds.size > 0 && verifiedRequirements.length > 0,
    complete_for_all_open_requirements: verifiedMappings.length > 0 && unmappedOpenRequirements.length === 0
  };
  return {
    ...
    enrollment_validation: {
      ...
      warnings: advisoryEnrollmentTerms.map((term) => `${term.schoolYear} ${term.term} has ${term.units} college units, above the ${term.selectedLimit}-unit recommended planning threshold.`)
    },
...
type AssistantUndo =
  ...
  | { kind: "restore_college_district_preference"; row: Record<string, unknown> | null; school_id: string; summary: string }
  ...
  | { kind: "activate_plan_version"; version_id: string; summary: string }
  | { kind: "remove_created_plan_version"; previous_version_id: string; created_version_id: string; summary: string }
  | { kind: "rename_plan_version"; version_id: string; label: string; summary: string }
  | { kind: "restore_archived_plan_version"; version_id: string; summary: string };

async function createAutomaticPlanBackup(
  ...
  const activeResult = await supabase.from("plan_versions").select("*").eq("id", activeVersionId).single();
  if (activeResult.error) throw new Error(`The safety backup could not read the active plan: ${activeResult.error.message}`);
  const value = await createOwnedPlanVersion(supabase, {
    userId: activeResult.data.user_id,
    activeVersion: activeResult.data as unknown as PlanVersion,
    label: `Before ${reason} · ${timestamp} · ${crypto.randomUUID().slice(0, 4)}`,
    sourceVersionId: activeVersionId,
    activate: false,
    startEmpty: false,
    role: "backup"
  });
  return z.uuid().parse(value.id);
}
...
export async function executeAssistantReadTool(
  ...
  if (name === "get_academic_context") {
    ...
    const planRows = workspace.planCourses.map((row) => ({
      ...
      smccd_course_id: row.smccd_course_id,
      course_code: row.smccd_course_id ? plannedCollegeById.get(row.smccd_course_id)?.course_code ?? null : null,
      ...
      requirement_area: row.requirement_area_override
    }));
    const currentGrade = workspace.settings.grade_level ?? 9;
    const academicTimeline = ([9, 10, 11, 12] as GradeLevel[]).flatMap((gradeLevel) =>
      (["fall", "spring", "summer"] as const).map((term) => {
        const rows = workspace.planCourses.filter((row) => row.grade_level === gradeLevel
          && (row.term === term || (row.term === "full_year" && (term === "fall" || term === "spring"))));
        const completed = rows.filter((row) => row.status === "completed");
        const imported = rows.filter((row) => Boolean(row.source_review_item_id));
        const pastGrade = gradeLevel < currentGrade;
        const completeFromRows = rows.length > 0 && completed.length === rows.length;
        const isComplete = pastGrade || completeFromRows;
        return {
          grade_level: gradeLevel,
          ...
          completed_course_count: completed.length,
          transcript_course_count: imported.length,
          is_complete: isComplete,
          completion_basis: pastGrade ? "past_grade" : completeFromRows ? "course_status" : "not_complete",
          needs_planning_attention: !isComplete && gradeLevel >= currentGrade && (term !== "summer" || rows.length > 0)
        };
      })
    );
    return {
      ...
      data: {
        student: {
          ...
          plan_span: { first_grade: 9, last_grade: 12 },
        ...
        plan: {
          ...
          active_version_id: workspace.activeVersion.id,
          active_version_name: workspace.activeVersion.label,
          fixed_grade_span: [9, 10, 11, 12],
          courses: planRows,
          academic_timeline: academicTimeline,
          transcript_course_count: planRows.filter((row) => row.transcript_locked).length,
          editable_course_count: planRows.filter((row) => !row.transcript_locked).length,
          completed_terms: academicTimeline.filter((term) => term.is_complete).map((term) => ({ grade_level: term.grade_level, term: term.term, basis: term.completion_basis })),
          terms_needing_attention: academicTimeline.filter((term) => term.needs_planning_attention).map((term) => ({ grade_level: term.grade_level, term: term.term }))
        },
        ...
        requested_plan_constraints: {
          start_grade: args.planning_start_grade ?? workspace.settings.grade_level,
  ...
  if (name === "get_plan_versions") {
    const args = toolArgumentSchemas.get_plan_versions.parse(argumentsValue);
    const versions = await listOwnedPlanVersions(supabase, userId, workspace.activeVersion);
    const visible = versions.filter((version) => args.include_backups
      || planVersionRole(version) === "plan"
      || version.kind === "active");
    return {
      summary: `Read ${visible.length} available four-year ${visible.length === 1 ? "plan" : "plans"}.`,
      data: visible.map((version) => ({
        version_id: version.id,
        label: version.label,
        active: version.kind === "active",
        role: planVersionRole(version),
        strategy: planVersionStrategy(version),
        course_count: Number(version.course_count ?? 0),
        updated_at: version.updated_at
      }))
    };
  }

  if (name === "compare_plan_versions") {
    const args = toolArgumentSchemas.compare_plan_versions.parse(argumentsValue);
    const versions = await listOwnedPlanVersions(supabase, userId, workspace.activeVersion);
    const versionById = new Map(versions.map((version) => [version.id, version]));
    if (args.version_ids.some((id) => !versionById.has(id))) throw new Error("One or more selected plans are unavailable.");
    const planResult = await supabase.from("plan_courses").select("*")
      .eq("user_id", userId).in("plan_version_id", args.version_ids);
    ...
    const baseRows = rows.filter((row) => row.plan_version_id === args.version_ids[0]);
    return {
      summary: `Compared ${args.version_ids.length} complete four-year plans.`,
      data: args.version_ids.map((versionId, index) => ({
        version_id: versionId,
        label: versionById.get(versionId)!.label,
        active: versionById.get(versionId)!.kind === "active",
        metrics: planVersionMetrics({
          rows: rows.filter((row) => row.plan_version_id === versionId),
          courses: planningWorkspace.courses,
          requirements: planningWorkspace.requirements,
          mappings: planningWorkspace.mappings,
          equivalencies: planningWorkspace.equivalencies,
          collegeCourses,
          goals: planningWorkspace.collegeGoals,
          programs: planningWorkspace.degreePrograms,
          degreeRequirements: planningWorkspace.degreeRequirements,
          degreeRequirementCourses: planningWorkspace.degreeRequirementCourses,
          manualCompletions: planningWorkspace.manualSmccdCompletions
        }),
        differences_from_baseline: index === 0 ? [] : planVersionDifferences({
          baseRows,
          targetRows: rows.filter((row) => row.plan_version_id === versionId),
          courses: planningWorkspace.courses,
          collegeCourses
        }).filter((difference) => difference.kind !== "removed")
      }))
    };
  }
  ...
  if (name === "search_course_catalog") {
    ...
    if ((source === "college" || source === "smccd" || source === "all") && matches.length < 10) {
      // A literal SQL title search cannot resolve ordinary student shorthand
      // such as "calc 2", "intercultural comm", or "eng c1000". Rank the
      // supported provider catalog with the same matcher used by batch
      // resolution so single- and multi-course requests behave identically.
      const catalogResult = await supabase.from(COLLEGE_DATA.courses).select(COLLEGE_COURSE_SELECT);
      ...
      const collegeQuery = String(argumentsValue.query ?? "")
        .replace(/\bcollege of san mateo\b/gi, "CSM")
  ...
  if (name === "resolve_academic_course_batch") {
    ...
    const smccdResult = needsSmccd ? await supabase.from(COLLEGE_DATA.courses).select(COLLEGE_COURSE_SELECT) : { data: [], error: null };
    ...
    const collegePreferenceScore = (course: SmccdCourse, query: string) => {
      const normalizedQuery = normalizedCatalogSearchText(query);
    ...
    for (const item of resolvedRequests) {
      ...
      if (!selectedCourse && !smccdCourse) {
        const existingSelected = request.source === "smccd" ? null : workspace.courses.find((course) =>
          ...
          && courseCatalogMatchScore(request.query, `${course.course_code ?? ""} ${course.name}`) >= 0
        );
        const existingSmccd = request.source === "selected_school" ? null : workspace.plannedSmccdCourses.find((course) =>
          courseCatalogMatchScore(request.query, `${course.course_code} ${course.title}`) >= 0
        );
      ...
      let recommendedLimitFallback: { entry: typeof entries[number]; row: PlanCourse } | null = null;
      let blockedReason = "No valid product placement was available.";
      for (const placement of placementCandidates) {
        ...
        if (policy && smccdCourse) {
          const enrollmentEvaluation = evaluateEnrollmentSchedule([...validationRows, candidateRow], policy).find((evaluation) =>
            evaluation.courseIds.includes(candidateRow.id)
          );
          if (enrollmentEvaluation?.state === "blocked") {
          ...
          if (args.respect_recommended_limit && enrollmentEvaluation?.state === "over_policy") {
            recommendedLimitFallback ??= { entry, row: candidateRow };
            continue;
          }
        }
        selectedEntry = entry;
      ...
      if (!selectedEntry && recommendedLimitFallback) {
        selectedEntry = recommendedLimitFallback.entry;
        selectedCandidateRow = recommendedLimitFallback.row;
      }
      if (!selectedEntry) {
  ...
  if (name === "get_course_schedule_options") {
    ...
    const generated = generateValidatedSchedule(planningWorkspace, policy, args.respect_recommended_limit, { interests: args.interests, rigor: args.rigor, maxCoursesPerTerm: args.max_courses_per_term, enforceSchoolCourseCounts: args.enforce_school_course_counts, startGrade: args.start_grade, startingMathCourse: args.starting_math_course, startingLanguageCourse: args.starting_language_course, includeCollegeCourses, replaceExisting: args.replace_existing, replaceGradeLevels: args.replace_grade_levels, objectives: args.objectives });
    ...
    return {
      summary: generated.additions.length || generated.degreeAdditions.length || generated.adjustments.length
          ...
          : `Evaluated the current four-year plan and found no additional selected-school courses that satisfy the requested scope.`,
      data: {
        target_plan_version_id: workspace.activeVersion.id,
        target_plan_name: workspace.activeVersion.label,
        respect_recommended_limit: args.respect_recommended_limit,
        requested_preferences: { interests: args.interests, rigor: args.rigor, max_courses_per_term: args.max_courses_per_term, enforce_school_course_counts: args.enforce_school_course_counts, start_grade: args.start_grade ?? workspace.settings.grade_level, starting_math_course: args.starting_math_course, starting_language_course: args.starting_language_course, include_college_courses: includeCollegeCourses, exclude_college_courses_explicitly: args.exclude_college_courses_explicitly, replace_existing: args.replace_existing, replace_grade_levels: args.replace_grade_levels, objectives: args.objectives },
  ...
  if (name === "get_prerequisite_evidence") {
    ...
    const [courseResult, catalogResult] = await Promise.all([
      supabase.from(COLLEGE_DATA.courses).select(COLLEGE_COURSE_SELECT).eq("id", args.course_id).maybeSingle(),
      supabase.from(COLLEGE_DATA.courses).select(COLLEGE_COURSE_SELECT)
    ]);
    if (courseResult.error) throw new Error(courseResult.error.message);
    if (catalogResult.error) throw new Error(catalogResult.error.message);
    if (!courseResult.data) throw new Error(`That course is not in the current ${workspace.school.short_name} or supported college catalog.`);
  ...
  if (name === "get_degree_progress") {
    ...
    if (!selectedGoal) return { summary: args.program_id ? "That degree is not bookmarked." : "No supported college degree is bookmarked.", data: null };
    const [programResult, requirementResult] = await Promise.all([
      supabase.from(COLLEGE_DATA.programs).select(COLLEGE_PROGRAM_SELECT).eq("id", selectedGoal.program_id).single(),
      supabase.from(COLLEGE_DATA.programRequirements).select("*").eq("program_id", selectedGoal.program_id).order("sort_order")
    ]);
    ...
    const catalogResults = await Promise.all([
      ...Array.from({ length: Math.ceil(optionCodes.length / 100) }, (_, index) => optionCodes.slice(index * 100, index * 100 + 100))
        .map((codes) => supabase.from(COLLEGE_DATA.courses).select(COLLEGE_COURSE_SELECT).in("course_code", codes)),
      supabase.from(COLLEGE_DATA.courses).select(COLLEGE_COURSE_SELECT).eq(COLLEGE_DATA.providerCodeColumn, programResult.data.college_code)
    ]);
  ...
  if (name === "get_college_goal") {
    if (!workspace.collegeGoals.length) return { summary: "No supported college degree is bookmarked.", data: [] };
    const programIds = workspace.collegeGoals.map((goal) => goal.program_id);
    const { data: programs, error } = await supabase.from(COLLEGE_DATA.programs).select("id,college_code,title,award_type,source_year").in("id", programIds);
    if (error) throw new Error(error.message);
  ...
  if (name === "search_smccd_programs" || name === "search_college_programs") {
    const args = toolArgumentSchemas.search_college_programs.parse(argumentsValue);
    const { data, error } = await supabase
      .from(COLLEGE_DATA.programs)
      .select(COLLEGE_PROGRAM_SEARCH_SELECT)
      .order("title")
      .limit(500);
    if (error) throw new Error(error.message);
    ...
    return { summary: `Found ${matches.length} supported college degree ${matches.length === 1 ? "program" : "programs"} matching ${args.query}.`, data: matches };
  }
...
export async function executeAssistantMutationTool(
  ...
  if (name === "update_plan_course") {
    ...
    // validation, prerequisite advisories, receipts, and undo from drifting.
  ...
  if (name === "create_plan_version") {
    const args = toolArgumentSchemas.create_plan_version.parse(argumentsValue);
    const created = await createOwnedPlanVersion(supabase, {
      userId,
      activeVersion: workspace.activeVersion,
      label: args.label,
      sourceVersionId: args.source_version_id ?? workspace.activeVersion.id,
      activate: args.activate,
      startEmpty: args.start_empty,
      role: "plan",
      strategy: args.strategy
    });
    const createdId = z.uuid().parse(created.id);
    const previousId = z.uuid().parse(created.previous_active_version_id);
    let populatedCourseCount = Number(created.course_count ?? 0);
    let populationWarning: string | null = null;
    if (args.populate_strategy && args.activate) {
      const rigor = args.strategy === "highest_gpa" ? "advanced" : "balanced";
      const objectives = [
        "complete_diploma",
        ...(args.strategy === "highest_gpa" ? ["maximize_weighted_gpa"] : []),
        ...(args.strategy === "degree_overlap" || args.strategy === "minimum_courses" ? ["maximize_degree_overlap"] : [])
      ];
      try {
        const createdWorkspace = await loadAssistantWorkspace(supabase, userId);
        const includeCollegeCourses = createdWorkspace.collegeGoals.length > 0;
        const planningWorkspace = await hydrateDegreePlanningCatalog(supabase, createdWorkspace, includeCollegeCourses);
        const policy = policyForPreference(planningWorkspace.enrollmentPolicies, planningWorkspace.enrollmentPreference);
        const generated = generateValidatedSchedule(planningWorkspace, policy, true, {
          interests: [], rigor, maxCoursesPerTerm: null, includeCollegeCourses, objectives
        });
        const scheduled = await executeAssistantMutationTool(supabase, userId, "add_course_schedule", {
          target_plan_version_id: createdId,
          course_ids: generated.additions.map((row) => row.course_id),
          respect_recommended_limit: true,
          enforce_school_course_counts: false,
          interests: [],
          rigor,
          max_courses_per_term: null,
          include_college_courses: includeCollegeCourses,
          exclude_college_courses_explicitly: false,
          replace_existing: false,
          replace_grade_levels: [],
          objectives
        }, context);
        const scheduledCourses = (scheduled.data as Record<string, unknown> | null)?.courses;
        populatedCourseCount = Array.isArray(scheduledCourses) ? scheduledCourses.length : populatedCourseCount;
      } catch (error) {
        populationWarning = error instanceof Error ? error.message : "The strategy schedule could not be populated.";
      }
    }
    return {
      summary: `${args.activate ? "Created and opened" : "Created"} “${args.label}”${args.populate_strategy ? ` and added ${populatedCourseCount} planned courses` : args.start_empty ? " with transcript history ready" : ` with ${Number(created.course_count ?? 0)} copied courses`}.${populationWarning ? ` The plan was created, but automatic population needs attention: ${populationWarning}` : ""}`,
      data: { version_id: createdId, label: args.label, active: args.activate, strategy: args.strategy, course_count: populatedCourseCount, population_warning: populationWarning },
      changed: { entity: "plan_version", id: createdId },
      undo: args.activate
        ? { kind: "remove_created_plan_version", previous_version_id: previousId, created_version_id: createdId, summary: `“${args.label}” was removed and the previous plan was reopened.` }
        : { kind: "delete_rows", table: "plan_versions", ids: [createdId], summary: `“${args.label}” was deleted.` }
    };
  }

  if (name === "merge_plan_versions") {
    const args = toolArgumentSchemas.merge_plan_versions.parse(argumentsValue);
    const result = await supabase.rpc("merge_plan_version_courses_v1", {
      p_source_version_id: args.source_version_id,
      p_target_version_id: args.target_version_id,
      p_source_course_ids: args.source_course_ids
    });
    if (result.error) throw new Error(result.error.message);
    const payload = result.data && typeof result.data === "object" && !Array.isArray(result.data)
      ? result.data as Record<string, unknown>
      : {};
    const insertedIds = z.array(z.uuid()).max(160).parse(payload.inserted_ids ?? []);
    const previousRows = z.array(z.record(z.string(), z.unknown())).max(160).parse(payload.previous_rows ?? []);
    const changedCount = Math.max(0, Number(payload.changed_count ?? args.source_course_ids.length));
    return {
      summary: `Merged ${changedCount} ${changedCount === 1 ? "course" : "courses"} into the selected plan.`,
      data: { source_version_id: args.source_version_id, target_version_id: args.target_version_id, courses_merged: changedCount },
      changed: { entity: "plan_version", id: args.target_version_id },
      undo: {
        kind: "restore_plan_patch",
        rows: previousRows,
        gpa_rows: [],
        inserted_plan_course_ids: insertedIds,
        summary: "The merged courses were removed and prior placements were restored."
      }
    };
  }

  if (name === "activate_plan_version") {
    const args = toolArgumentSchemas.activate_plan_version.parse(argumentsValue);
    const activated = await activateOwnedPlanVersion(supabase, userId, workspace.activeVersion, args.version_id);
    const previousId = z.uuid().parse(activated.previous_active_version_id);
    return {
      summary: `Opened “${String(activated.label)}” as the active four-year plan.`,
      data: { version_id: args.version_id, label: activated.label, previous_active_version_id: previousId },
      changed: { entity: "plan_version", id: args.version_id },
      undo: { kind: "activate_plan_version", version_id: previousId, summary: "The previously active four-year plan was reopened." }
    };
  }

  if (name === "rename_plan_version") {
    const args = toolArgumentSchemas.rename_plan_version.parse(argumentsValue);
    const previous = await supabase.from("plan_versions").select("id,label").eq("id", args.version_id).eq("user_id", userId).maybeSingle();
    if (previous.error || !previous.data) throw new Error("That plan is unavailable.");
    await renameOwnedPlanVersion(supabase, userId, args.version_id, args.label);
    return {
      summary: `Renamed “${previous.data.label}” to “${args.label}”.`,
      data: { version_id: args.version_id, label: args.label },
      changed: { entity: "plan_version", id: args.version_id },
      undo: { kind: "rename_plan_version", version_id: args.version_id, label: previous.data.label, summary: `The plan was renamed back to “${previous.data.label}”.` }
    };
  }

  if (name === "archive_plan_version") {
    const args = toolArgumentSchemas.archive_plan_version.parse(argumentsValue);
    const archived = await archiveOwnedPlanVersion(supabase, userId, args.version_id) as Record<string, unknown>;
    return {
      summary: `Deleted “${String(archived.label)}”.`,
      data: { version_id: args.version_id, label: archived.label },
      changed: { entity: "plan_version", id: args.version_id },
      undo: { kind: "restore_archived_plan_version", version_id: args.version_id, summary: `“${String(archived.label)}” was restored.` }
    };
  }
  ...
  if (name === "set_current_school") {
    ...
    if (error) throw new Error(error.message);
    return {
      summary: `${school.name} is now the selected high school. Its saved academic workspace was restored; the previous school's data remains saved separately.`,
  ...
  if (name === "set_college_district_preference") {
    ...
    return {
      ...
      undo: { kind: "restore_college_district_preference", row: workspace.collegeDistrictPreference as unknown as Record<string, unknown> | null, school_id: workspace.school.id, summary: "The previous college-district preference was restored." }
    };
  }

  if (name === "add_course_schedule") {
    const args = toolArgumentSchemas.add_course_schedule.parse(argumentsValue);
    if (args.target_plan_version_id !== workspace.activeVersion.id) {
      throw new Error(`The active plan changed before this schedule could be applied. Refresh the academic context and apply it to the currently active named plan “${workspace.activeVersion.label}”.`);
    }
    const includeCollegeCourses = !args.exclude_college_courses_explicitly
      && (args.include_college_courses || workspace.collegeGoals.length > 0);
    const planningWorkspace = await hydrateDegreePlanningCatalog(supabase, workspace, includeCollegeCourses);
    ...
    const automaticBackupId = args.replace_existing && workspace.planCourses.some((row) => !row.source_review_item_id)
      ? await createAutomaticPlanBackup(supabase, workspace.activeVersion.id, "Pilot schedule rebuild")
      : null;
    ...
    const collegeInsertRows = available.degreeAdditions.map((row, index) => {
      ...
      return {
        ...
        course_id: null,
        smccd_course_id: row.smccd_course_id,
        college_provider_code: planningWorkspace.collegeDistrictPreference?.district_code ?? "SMCCD",
    ...
    if (args.replace_existing) {
      const replacement = await supabase.rpc("replace_pilot_course_schedule_v2", {
        p_target_plan_version_id: args.target_plan_version_id,
    ...
    const persistedResult = await supabase.from("plan_courses").select("*")
      .eq("user_id", userId)
      .eq("plan_version_id", workspace.activeVersion.id);
    ...
    if (persistedInsertedCount !== insertRows.length || persistedLimitFailures.length) {
      ...
      const reason = persistedInsertedCount !== insertRows.length
        ...
        : persistedLimitFailures.map((term) => `${term.schoolYear} ${term.term} saved ${term.units} college units above the ${term.absoluteLimit}-unit absolute limit`).join("; ");
    ...
    return {
      ...
      data: {
        ...
        absolute_max_units: policy?.absolute_max_units ?? null,
        automatic_backup_id: automaticBackupId
      },
  ...
  if (name === "add_dtech_course" || name === "add_high_school_course") {
    ...
    const prerequisiteWarning = prerequisiteWarningDetail(prerequisite);
    const mappingVerified = workspace.mappings.some((mapping) => mapping.course_id === course.id && mapping.confidence === "verified");
    const enrollmentPolicy = policyForPreference(workspace.enrollmentPolicies, workspace.enrollmentPreference);
    const candidateRow = assistantPlanCourseCandidate(workspace, {
      source: "selected_school",
      course_id: course.id,
      status: args.status,
      grade_level: args.grade_level,
      term: args.term
    }, null, workspace.planCourses.length);
    const enrollmentEvaluation = enrollmentPolicy
      ? evaluateEnrollmentSchedule([...workspace.planCourses, candidateRow], enrollmentPolicy)
        .find((evaluation) => evaluation.courseIds.includes(candidateRow.id))
      : null;
    if (enrollmentEvaluation?.state === "blocked") throw new Error("That placement would exceed the provider's absolute college enrollment limit.");
    ...
    return {
      ...
      data: {
        ...
        status: args.status,
        grade_level: args.grade_level,
        prerequisite_warning: prerequisiteWarning,
        enrollment_warning: enrollmentEvaluation?.state === "over_policy"
          ? `${enrollmentEvaluation.schoolYear} ${enrollmentEvaluation.term} is above the recommended college-unit threshold.`
          : null
      },
  ...
  if (name === "add_smccd_course") {
    ...
    assertPlanningTermExists(args.grade_level, args.term);
    const [courseResult, catalogResult] = await Promise.all([
      supabase.from(COLLEGE_DATA.courses).select(COLLEGE_COURSE_SELECT).eq("id", args.course_id).single(),
      supabase.from(COLLEGE_DATA.courses).select(COLLEGE_COURSE_SELECT)
    ]);
    if (courseResult.error) throw new Error("That supported college catalog course is no longer available.");
    if (catalogResult.error) throw new Error(catalogResult.error.message);
    const course = courseResult.data as unknown as SmccdCourse;
    ...
    const candidateRow = assistantPlanCourseCandidate(workspace, {
      ...
      grade_level: args.grade_level,
      term: args.term
    }, course, workspace.planCourses.length);
    const enrollmentEvaluation = enrollmentPolicy
      ? evaluateEnrollmentSchedule([...workspace.planCourses, candidateRow], enrollmentPolicy)
        .find((evaluation) => evaluation.courseIds.includes(candidateRow.id))
      : null;
    if (enrollmentEvaluation?.state === "blocked") throw new Error("That placement would exceed the provider's absolute college enrollment limit.");
    const enrollmentWarning = enrollmentEvaluation?.state === "over_policy"
      ? `${enrollmentEvaluation.schoolYear} ${enrollmentEvaluation.term} has ${enrollmentEvaluation.units} college units, above the recommended ${enrollmentEvaluation.selectedLimit}-unit threshold.`
      : null;
    const { data, error } = await supabase.from("plan_courses").insert({
      plan_version_id: workspace.activeVersion.id,
      user_id: userId,
    ...
    return {
      ...
      data: {
        ...
        prerequisite_warning: prerequisiteWarning,
        enrollment_warning: enrollmentWarning
      },
      changed: { entity: "plan_course", id: data.id },
  ...
  if (name === "add_custom_course") {
    ...
    const schoolYear = schoolYearForGrade(workspace.settings.graduation_year ?? new Date().getFullYear() + 3, args.grade_level);
    const candidateRow: PlanCourse = {
      id: `candidate:custom:${crypto.randomUUID()}`,
      plan_version_id: workspace.activeVersion.id,
      user_id: userId,
      course_id: null,
      smccd_course_id: null,
      college_provider_code: args.college_units ? "CUSTOM" : null,
    ...
    const enrollmentPolicy = policyForPreference(workspace.enrollmentPolicies, workspace.enrollmentPreference);
    const enrollmentEvaluation = enrollmentPolicy && Number(args.college_units ?? 0) > 0
      ? evaluateEnrollmentSchedule([...workspace.planCourses, candidateRow], enrollmentPolicy)
        .find((evaluation) => evaluation.courseIds.includes(candidateRow.id))
      : null;
    if (enrollmentEvaluation?.state === "blocked") throw new Error("That placement would exceed the provider's absolute college enrollment limit.");
  ...
  if (name === "add_academic_courses") {
    ...
    const smccdCatalogResult = smccdIds.length ? await supabase.from(COLLEGE_DATA.courses).select(COLLEGE_COURSE_SELECT) : { data: [], error: null };
    ...
    for (const [index, entry] of args.entries.entries()) {
      ...
      if (entry.source === "selected_school") {
        ...
        const prerequisiteWarning = prerequisiteWarningDetail(prerequisite);
        if (prerequisiteWarning) prerequisiteWarnings.push(`${course.name}: ${prerequisiteWarning}`);
        row = {
          ...base,
          course_id: course.id,
          credits: course.credits,
        ...
        const prerequisiteWarning = prerequisiteWarningDetail(prerequisite);
        if (prerequisiteWarning) prerequisiteWarnings.push(`${course.course_code}: ${prerequisiteWarning}`);
        const normalizedCode = normalizeCollegeCourseCode(course.course_code);
    ...
    const policy = policyForPreference(workspace.enrollmentPolicies, workspace.enrollmentPreference);
    const enrollmentEvaluations = policy ? evaluateEnrollmentSchedule(validationRows, policy) : [];
    const absoluteViolations = enrollmentEvaluations.filter((evaluation) => evaluation.state === "blocked");
    if (absoluteViolations.length) throw new Error("The mixed schedule exceeds an absolute college enrollment limit in one or more terms.");
    const enrollmentWarnings = enrollmentEvaluations
      .filter((evaluation) => evaluation.state === "over_policy")
      .map((evaluation) => `${evaluation.schoolYear} ${evaluation.term} has ${evaluation.units} college units, above the recommended ${evaluation.selectedLimit}-unit threshold.`);
    ...
    return {
      ...
      data: {
        ...
        prerequisite_warnings: prerequisiteWarnings,
        respected_recommended_limit: args.respect_recommended_limit && enrollmentWarnings.length === 0,
        enrollment_warnings: enrollmentWarnings
      },
  ...
  if (name === "update_plan_courses") {
    ...
    const needsSmccdCatalog = args.patches.some((patch) => Boolean(patch.smccd_course_id)
      || (!patch.remove && Boolean(workspace.planCourses.find((row) => row.id === patch.plan_course_id)?.smccd_course_id)))
      || args.additions.some((entry) => entry.source === "smccd");
    const smccdCatalogResult = needsSmccdCatalog ? await supabase.from(COLLEGE_DATA.courses).select(COLLEGE_COURSE_SELECT) : { data: [], error: null };
    if (smccdCatalogResult.error) throw new Error(smccdCatalogResult.error.message);
    const smccdCatalog = (smccdCatalogResult.data ?? []) as unknown as SmccdCourse[];
    const smccdById = new Map(smccdCatalog.map((course) => [course.id, course]));
    if (args.patches.some((patch) => patch.smccd_course_id && !smccdById.has(patch.smccd_course_id))) throw new Error("One or more college replacement courses are no longer in the current catalog.");
    if (args.additions.some((entry) => entry.source === "smccd" && !smccdById.has(entry.course_id))) throw new Error("One or more college additions are no longer in the current catalog.");
    if (args.additions.some((entry) => entry.source === "selected_school" && !workspace.courses.some((course) => course.id === entry.course_id))) throw new Error("One or more high-school additions are no longer in the selected-school catalog.");
    const patchById = new Map(args.patches.map((patch) => [patch.plan_course_id, patch]));
    const patchedRows = workspace.planCourses.filter((row) => !patchById.get(row.id)?.remove).map((row) => {
      ...
      const collegeReplacement = patch.smccd_course_id ? smccdById.get(patch.smccd_course_id) : null;
      const normalizedCollegeCode = collegeReplacement ? normalizeCollegeCourseCode(collegeReplacement.course_code) : null;
      const equivalency = normalizedCollegeCode
        ? workspace.equivalencies.find((candidate) => candidate.normalized_course_code === normalizedCollegeCode)
        : null;
      const collegeUnits = collegeReplacement ? Number(collegeReplacement.units_max ?? collegeReplacement.units_min) : null;
      const collegeCredits = collegeReplacement && collegeUnits !== null
        ? resolveCollegeHighSchoolCredits({
            collegeUnits,
            storedHighSchoolCredits: null,
            equivalencyHighSchoolCredits: equivalency?.high_school_credits,
            normalizedCourseCode: normalizedCollegeCode
          }).credits
        : null;
      ...
      return {
        ...
        ...(collegeReplacement ? {
          course_id: null,
          smccd_course_id: collegeReplacement.id,
          college_provider_code: "SMCCD",
          custom_course_name: `${collegeReplacement.course_code} ${collegeReplacement.title}`,
          credits: patch.credits ?? collegeCredits,
          college_units: patch.college_units ?? collegeUnits,
          is_weighted: patch.is_weighted ?? true,
          mapping_verified: Boolean(equivalency),
          requirement_area_override: equivalency?.requirement_area ?? null,
          notes: equivalency
            ? `${collegeReplacement.college_code} ${collegeReplacement.source_year} catalog; verified selected-school equivalency: ${equivalency.high_school_equivalent}.`
            : `${collegeReplacement.college_code} ${collegeReplacement.source_year} catalog; high-school credit follows the college-credit conversion policy.`
        } : {}),
        grade_level: gradeLevel,
        school_year: schoolYearForGrade(workspace.settings.graduation_year ?? new Date().getFullYear() + 3, gradeLevel),
        ...
        ...(!replacement && !collegeReplacement && patch.credits !== undefined ? { credits: patch.credits } : {}),
        ...(!replacement && !collegeReplacement && patch.college_units !== undefined ? { college_units: patch.college_units } : {}),
        ...(!replacement && !collegeReplacement && patch.is_weighted !== undefined ? { is_weighted: patch.is_weighted } : {}),
        user_edited: true
      };
    });
    const additionRows = args.additions.map((entry, index): PlanCourse => {
      const selectedCourse = entry.source === "selected_school" ? workspace.courses.find((course) => course.id === entry.course_id)! : null;
      const collegeCourse = entry.source === "smccd" ? smccdById.get(entry.course_id)! : null;
      const normalizedCollegeCode = collegeCourse ? normalizeCollegeCourseCode(collegeCourse.course_code) : null;
      const equivalency = normalizedCollegeCode
        ? workspace.equivalencies.find((candidate) => candidate.normalized_course_code === normalizedCollegeCode)
        : null;
      const collegeUnits = collegeCourse ? Number(collegeCourse.units_max ?? collegeCourse.units_min) : null;
      const credits = selectedCourse?.credits ?? (collegeCourse && collegeUnits !== null
        ? resolveCollegeHighSchoolCredits({
            collegeUnits,
            storedHighSchoolCredits: null,
            equivalencyHighSchoolCredits: equivalency?.high_school_credits,
      ...
      return {
        ...
        custom_course_name: collegeCourse ? `${collegeCourse.course_code} ${collegeCourse.title}` : null,
        grade_level: entry.grade_level,
        school_year: schoolYearForGrade(workspace.settings.graduation_year ?? new Date().getFullYear() + 3, entry.grade_level),
        term: entry.term,
        status: entry.status,
        credits,
        college_units: collegeUnits ?? selectedCourse?.college_units ?? null,
        letter_grade: null,
        is_weighted: Boolean(collegeCourse) || Boolean(selectedCourse?.is_weighted),
        mapping_verified: collegeCourse ? Boolean(equivalency) : workspace.mappings.some((mapping) => mapping.course_id === selectedCourse?.id && mapping.confidence === "verified"),
        user_edited: true,
        notes: [
          collegeCourse ? `${collegeCourse.college_code} ${collegeCourse.source_year} catalog${equivalency ? `; verified selected-school equivalency: ${equivalency.high_school_equivalent}` : ""}.` : null,
          entry.prerequisite_override_reason ? `Student-provided prerequisite override (unverified): ${entry.prerequisite_override_reason}` : null
        ].filter(Boolean).join(" ") || null,
        sort_order: workspace.planCourses.length + index,
        source_review_item_id: null,
        smccd_course_id: collegeCourse?.id ?? null,
        college_provider_code: collegeCourse ? "SMCCD" : null,
        requirement_area_override: equivalency?.requirement_area ?? null
      };
    });
    const nextRows = [...patchedRows, ...additionRows];
    const prerequisiteWarnings: string[] = [];
    ...
    const changedSmccdRowIds = new Set([
      ...args.patches.filter((patch) => patch.smccd_course_id).map((patch) => patch.plan_course_id),
      ...additionRows.filter((row) => row.smccd_course_id).map((row) => row.id)
    ]);
    for (const id of changedSmccdRowIds) {
      const next = nextRows.find((row) => row.id === id)!;
      const collegeCourse = smccdById.get(next.smccd_course_id!)!;
      const otherRows = nextRows.filter((row) => row.id !== next.id);
      if (smccdCourseAlreadyInPlanIndex(collegeCourse, createSmccdPlanCourseIndex(otherRows, smccdCatalog))) throw new Error(`${collegeCourse.course_code} is already represented in the plan.`);
    }
    for (const patch of args.patches) {
      ...
      const collegeCourse = next.smccd_course_id ? smccdById.get(next.smccd_course_id) : null;
      ...
      if (selectedCourse) {
        ...
        const prerequisiteWarning = prerequisiteWarningDetail(prerequisite);
        if (prerequisiteWarning) prerequisiteWarnings.push(`${selectedCourse.name}: ${prerequisiteWarning}`);
      } else if (collegeCourse && (patch.smccd_course_id || original.smccd_course_id) && (patch.smccd_course_id || patch.grade_level !== undefined || patch.term !== undefined)) {
        const prerequisite = evaluateSmccdPlannerPrerequisites(collegeCourse, { gradeLevel: next.grade_level, term: next.term, instanceId: next.id }, smccdCatalog, nextRows, workspace.courses);
        const prerequisiteWarning = prerequisiteWarningDetail(prerequisite);
        if (prerequisiteWarning) prerequisiteWarnings.push(`${collegeCourse.course_code}: ${prerequisiteWarning}`);
      }
    }
    for (const entry of args.additions) {
      const next = additionRows.find((row) => (entry.source === "smccd" ? row.smccd_course_id : row.course_id) === entry.course_id)!;
      assertPlanningTermExists(next.grade_level, next.term);
      const selectedCourse = next.course_id ? workspace.courses.find((course) => course.id === next.course_id) : null;
      const collegeCourse = next.smccd_course_id ? smccdById.get(next.smccd_course_id) : null;
      if (selectedCourse) {
        if (selectedCourse.grade_levels.length && !selectedCourse.grade_levels.includes(next.grade_level) && !entry.prerequisite_override_reason) throw new Error(`${selectedCourse.name} is not offered in grade ${next.grade_level}. The student must explicitly correct or override that placement evidence to continue.`);
        if (selectedCourse.term_type === "year" && next.term !== "full_year") throw new Error(`${selectedCourse.name} is a full-year course.`);
        const prerequisite = evaluateSelectedSchoolPlannerPrerequisites(selectedCourse, { gradeLevel: next.grade_level, term: next.term, instanceId: next.id }, workspace.courses, nextRows, smccdCatalog, workspace.equivalencies, `${workspace.school.name} official course catalog`);
        const prerequisiteWarning = prerequisiteWarningDetail(prerequisite);
        if (prerequisiteWarning) prerequisiteWarnings.push(`${selectedCourse.name}: ${prerequisiteWarning}`);
    ...
    const policy = policyForPreference(workspace.enrollmentPolicies, workspace.enrollmentPreference);
    let advisoryEnrollmentTerms: ReturnType<typeof evaluateEnrollmentSchedule> = [];
    if (policy) {
      const priorTerms = new Map(evaluateEnrollmentSchedule(workspace.planCourses, policy).map((term) => [`${term.schoolYear}:${term.term}`, term]));
      const worsened = evaluateEnrollmentSchedule(nextRows, policy).filter((term) => {
        if (term.state !== "blocked") return false;
        const prior = priorTerms.get(`${term.schoolYear}:${term.term}`);
        return !prior || term.units > prior.units || (term.state === "blocked" && prior.state !== "blocked");
      });
      if (worsened.length) throw new Error("The requested edit would exceed an absolute enrollment limit in one or more terms.");
      advisoryEnrollmentTerms = evaluateEnrollmentSchedule(nextRows, policy).filter((term) => term.state === "over_policy");
    }
    ...
    const additionPayload = additionRows.map((row) => ({
      id: row.id,
      course_id: row.course_id,
      smccd_course_id: row.smccd_course_id,
      college_provider_code: row.college_provider_code,
    ...
    const automaticBackupId = args.patches.length + args.additions.length >= 8
      ? await createAutomaticPlanBackup(supabase, workspace.activeVersion.id, "Pilot course update")
      : null;
    const result = await supabase.rpc("apply_pilot_plan_course_patch_v2", { p_edits: editRows, p_additions: additionPayload });
    if (result.error) throw new Error(result.error.message);
    const courseMap = new Map(workspace.courses.map((course) => [course.id, course]));
    const changedRows = nextRows.filter((row) => patchById.has(row.id) || additionRows.some((addition) => addition.id === row.id));
    const advisory = advisoryEnrollmentTerms.length
      ? ` ${advisoryEnrollmentTerms.map((term) => `${term.schoolYear} ${term.term} is ${term.units} units`).join("; ")}, above the recommended planning threshold but below the absolute limit.`
      : "";
    return {
      summary: `${args.patches.length} existing ${args.patches.length === 1 ? "course was" : "courses were"} updated or removed and ${additionRows.length} ${additionRows.length === 1 ? "course was" : "courses were"} added without changing unrelated plan rows.${advisory}`,
      data: {
        updated_count: args.patches.length,
        added_count: additionRows.length,
        courses: changedRows.map((row) => courseDisplayName(row, courseMap)),
        plan_course_ids: [...rows.map((row) => row.id), ...additionRows.map((row) => row.id)],
        enrollment_advisories: advisoryEnrollmentTerms.map((term) => ({ school_year: term.schoolYear, term: term.term, units: term.units, recommended_limit: term.selectedLimit })),
        prerequisite_warnings: prerequisiteWarnings,
        automatic_backup_id: automaticBackupId
      },
      changed: { entity: "plan_courses", id: [...rows.map((row) => row.id), ...additionRows.map((row) => row.id)].join(",") },
      undo: {
        kind: "restore_plan_patch",
        rows: rows as unknown as Array<Record<string, unknown>>,
        gpa_rows: workspace.gpaScenarioChoices.filter((choice) => rows.some((row) => row.id === choice.plan_course_id)) as unknown as Array<Record<string, unknown>>,
  ...
  if (name === "save_prerequisite_evidence") {
    ...
    const courseResult = await supabase.from(COLLEGE_DATA.courses).select("id,course_code,title,college_code").eq("id", args.target_course_id).maybeSingle();
    if (courseResult.error) throw new Error(courseResult.error.message);
    if (!courseResult.data) throw new Error("That target course is not in the current supported college catalog.");
  ...
  if (name === "create_plan_snapshot") {
    ...
    const created = await createOwnedPlanVersion(supabase, {
      ...
      activeVersion: workspace.activeVersion,
      label: args.label,
      sourceVersionId: workspace.activeVersion.id,
      activate: false,
      startEmpty: false,
      role: "backup"
    });
    const createdId = z.uuid().parse(created.id);
    const copiedCount = Number(created.course_count ?? 0);
    return {
      summary: `Saved “${args.label}” with ${copiedCount} courses.`,
      data: { version_id: createdId, label: args.label, course_count: copiedCount },
      changed: { entity: "plan_version", id: createdId },
      undo: { kind: "delete_rows", table: "plan_versions", ids: [createdId], summary: `The “${args.label}” snapshot was deleted.` }
    };
  }
  ...
  if (name === "set_college_goals") {
    ...
    const programResult = await supabase.from(COLLEGE_DATA.programs)
    ...
    if (programs.length !== args.program_ids.length) {
      throw new Error("One or more requested college degree programs are no longer available.");
    }
  ...
  if (name === "clear_academic_plan") {
    ...
    const automaticBackupId = args.courses && workspace.planCourses.some((row) => !row.source_review_item_id)
      ? await createAutomaticPlanBackup(supabase, workspace.activeVersion.id, "Pilot plan clear")
      : null;
