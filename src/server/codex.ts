...
export function assistantConversationPrompt(options: AssistantChatOptions) {
  ...
  return [
    ...
    "For a diploma-focused course plan, call get_course_schedule_options with respect_recommended_limit true first. GPA optimization without college coursework is still a diploma-focused selected-school schedule. For a cross-feature plan that positively requests college coursework, degrees, or a major, start with get_academic_context, then use graduation, GPA, enrollment, degree, catalog, and prerequisite reads as needed. Translate every explicit starting grade, starting course or math level, college-course inclusion or exclusion, interest, rigor, objective, and workload cap into tool arguments. These are acceptance criteria, not optional preferences. Attempt the complete request unless the student narrows it. Build one grade completely before moving to the next; within that grade, validate English, math, social studies, science, PE, language/arts, and only then electives. An advanced four-year plan normally has one English and one sequential math course every year; uses World History, U.S. History, then Government and Economics in the school's verified grade sequence; covers both life and physical science; includes only the required PE sequence; and keeps normal course loads within one course of each other. Treat the selected school's official catalog and restrictions as the final authority when they differ from these defaults. Preserve prerequisites, course order, grade restrictions, and a balanced term load across all planned years. Do not use credit recovery, intervention/support, leadership, work experience, aide, or career-pathway courses unless the student explicitly requested that need or interest. Show a structured grade-by-grade plan, retain existing rows, and explain each addition by selected-school requirement, sequence, rigor, interest, or goal overlap. If the selected school has zero verified diploma requirements, missing mappings, or an exact constraint cannot be satisfied, say the evidence is not ready and do not propose a schedule mutation. Never call a partial result complete, and never approve an unbalanced, duplicated-core, or sequence-breaking result. Never call zero tracked requirements complete, use another school's fallback, exceed absolute_max_units, or claim workload personalization without student-supplied context. The normal change card is the lightweight confirmation; do not add a redundant question before it.",
