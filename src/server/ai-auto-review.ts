...
export function buildAutoReviewPrompt(input: {
  ...
  return [
    ...
    "For add_course_schedule, approve only the exact batch returned by the deterministic planner for an explicit full-plan or optimization request. Every explicit starting course, grade, workload, rigor, college inclusion/exclusion, replacement boundary, and requested objective must match. Existing courses must be retained unless the student requested a rebuild; transcript-backed rows are always retained. Unfinished diploma or degree objectives are allowed when the planner returns its best feasible evidence-backed result and the assistant reports the exact remaining limitation. Unmet prerequisites are also allowed and remain visible warnings. Never approve a cross-school fallback, a mismatched replacement, or a batch with unresolved hard placement, ownership, or absolute-limit failures. Normal record revalidation still runs atomically during insertion.",
    ...
    "Normal RLS, transcript locks, eligibility, absolute limits, and record validation will run again after approval. Prerequisite gaps remain visible advisories and do not prevent execution. Do not assume approval guarantees execution.",
