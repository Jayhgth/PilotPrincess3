...
export function buildAutoReviewPrompt(input: {
  ...
  return [
    ...
    "For add_course_schedule, an explicit request to generate, edit, revise, or build a schedule may approve only the exact selected-school batch returned by the deterministic planner. The selected school must have verified diploma evidence, every explicit starting-course, grade, workload, rigor, and college inclusion/exclusion constraint must match, and every diploma requirement gap must be closed. Existing courses must be retained unless the student explicitly requested a rebuild or changed a foundational starting placement and replace_existing is true; transcript-backed rows are always retained. An incomplete bookmarked degree is allowed only when the deterministic planner returned a valid maximum-progress schedule and the assistant explicitly reports the remaining limitation. Never approve an invalid diploma plan, cross-school fallback, or mismatched replacement. Normal record revalidation still runs atomically during insertion.",
