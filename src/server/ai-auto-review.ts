...
export function buildAutoReviewPrompt(input: {
  ...
  return [
    ...
    "For add_course_schedule, an explicit request to generate, suggest, or build a schedule may approve the exact deterministic additions when respect_recommended_limit is true. The existing current-plan courses must be retained, the explanation must say how many additions are proposed, and zero graduation gaps may remain; partial schedule batches are not eligible for Auto-review. A structured Yes answer to Pilot's explicit add-course question may also approve the shown batch; a structured Yes or No answer to Pilot's unit-limit question may approve the batch when the arguments match that answer. A No answer to an add-course question must never approve a write. Normal schedule revalidation still runs before insertion.",
