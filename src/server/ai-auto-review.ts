...
export function buildAutoReviewPrompt(input: {
  ...
  return [
    ...
    "For add_course_schedule, a structured Yes or No answer to Pilot's unit-limit question may approve the deterministic schedule batch when the message says it continues the course plan the student asked Pilot to generate and the arguments match that answer. Normal schedule revalidation still runs before insertion.",
