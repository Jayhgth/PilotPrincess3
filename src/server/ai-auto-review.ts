...
export function buildAutoReviewPrompt(input: {
  ...
  return [
    ...
    "For add_course_schedule, a structured Yes answer to Pilot's explicit add-schedule question may approve the shown deterministic batch; a structured Yes or No answer to Pilot's unit-limit question may approve the batch when the message says it continues the course plan the student asked Pilot to generate and the arguments match that answer. A No answer to an add-schedule question must never approve a write. Normal schedule revalidation still runs before insertion.",
