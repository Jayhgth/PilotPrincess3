// Site-tuned adaptation of React Bits Hyperspeed (David Haz),
// MIT + Commons Clause. Adds reduced-motion and visibility controls.
...
const Hyperspeed: FC<HyperspeedProps> = ({ className = '', effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    const updateAnimationState = () => {
      ...
      if (visible && !document.hidden) myApp.start();
      else myApp.stop();
    };
    ...
    const resizeObserver = typeof ResizeObserver === 'undefined' ? null : new ResizeObserver(() => {
      ...
      if (reduceMotion) myApp.render(0);
    });
    resizeObserver?.observe(container);

    if (active) {
      ready = true;
      myApp.init(!reduceMotion && visible && !document.hidden);
    }
  ...
  return <div aria-hidden="true" className={`hyperspeed-container ${className}`.trim()} ref={hyperspeed} />;
};
